...
namespace LeapfrogEditor
{
   ...
   public class VmCollection<TViewModel, TModel> : ObservableCollection<TViewModel>
       where TViewModel : class, INotifyPropertyChanged /* Used to be IViewModel */ 
       where TModel : class
      ...
      private void ViewModelCollectionChanged(object sender, NotifyCollectionChangedEventArgs e)
      {
         ...
         switch (e.Action)
         {
            case NotifyCollectionChangedAction.Add:
               foreach (var m in e.NewItems.OfType<TModel>() /*.OfType<IViewModel>().Select(v => v.Model).OfType<TModel>()*/)
                  _models.Add(m);
               break;

            case NotifyCollectionChangedAction.Remove:
               foreach (var m in e.OldItems.OfType<TModel>() /* OfType<IViewModel>().Select(v => v.Model).OfType<TModel>()*/)
                  _models.Remove(m);
               break;
